...
@admin_bp.route('/crawled-novels', methods=['GET'])
@admin_required
def get_crawled_novels():
    """
    Get crawled novels
    ...
    result = AdminService.get_crawled_novels(status=status, page=page, per_page=per_page)
    
    return jsonify(result)

@admin_bp.route('/crawled-novels/<int:novel_id>/chapters', methods=['GET'])
@admin_required
def get_crawled_chapters(novel_id):
    """
    ...
    """
    result = AdminService.get_crawled_chapters(novel_id=novel_id)
    
    return jsonify(result)

@admin_bp.route('/crawled-novels/<int:novel_id>', methods=['POST'])
@admin_required
def manage_crawled_novel(novel_id):
    """
    Approve or reject a crawled novel
    
    POST params:
    - action: "approve" or "reject"
    - reason: Reason for rejection (required for "reject" action)
    """
    # Get request data
    data = request.json
    if not data:
